...
function toggleAstroAdapter(comment: boolean) {
  ...
  const importIndex = astroConfig.findIndex((line) => line.trim().includes('import') && line.includes('vercel'))

  // Find the adapter line (including commented lines)
  const adapterIndex = astroConfig.findIndex((line) => line.trim().includes('adapter:') && line.includes('vercel'))
